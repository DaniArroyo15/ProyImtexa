...
namespace General.Librerias.CodigoUsuario
{
    public class WordMemory
    {
        ...
        private string sOrient;

        private List<Archivos> LstArchivos = new List<Archivos>();

        public byte[] Exportar(DataSet dst, string titulo = "", string pOrient = "", string colorFondoCabecera = "BFBFBF")
        {
        ...
        private void getWordDocument(MemoryStream ms)
        {
            ...
            using (StreamWriter sw = new StreamWriter(ms))
            {
                ...
                if (sOrient == "H")
                {
                    sw.Write("<w:pgSz w:w='16838' w:h='11906' w:orient='landscape'/>");
                    sw.Write("<w:pgMar w:top='1701' w:right='1417' w:bottom='1701' w:left='1417' w:header='708' w:footer='708' w:gutter='0'/>");
                }
                else {
