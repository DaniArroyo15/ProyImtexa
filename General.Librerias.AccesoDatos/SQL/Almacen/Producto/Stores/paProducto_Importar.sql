...
BEGIN        
	...
	BEGIN TRY
		...
		--
		INSERT INTO	dbo.Almacen_Producto 
					(
					 vAlias
					,vDescripcion
					,FK_eCategoria
		...
		SELECT		 LTRIM(RTRIM(Campo1))
					,LTRIM(RTRIM(Campo2))
					,(SELECT PK_eCategoria FROM dbo.Almacen_Categoria WHERE vAlias = LTRIM(RTRIM(Campo3)))
					,LTRIM(RTRIM(Campo4))
					,@veIdUsuario
					,GETDATE()
...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'paProducto_Importar')
BEGIN
	--GRANT EXECUTE ON dbo.paProducto_Importar TO dbUser;
	PRINT 'SP Create : paProducto_Importar';
	PRINT '***'
END
GO
